...
% To have a successfull compilation of a network it is important to understand which parts of network won't lose much accuracy due to quantisation.
...
% Interestingly TinyCLIP-30M which has a lower parameter count than RN50 but still retained a higher accuracy after quantisation.
...
% On the other hand TinyClip-30M surprisingly retaind the most part of its accuracy through quantisation although having less parameters than RN50.
% This indicates that the steps taken to get the TinyCLIP models work even beyond quantisation if the original model has enough capacity.

% There has to menationd that all the models lack of a satisfactory accuracy after quantisation.
% The taken measures to increase this accuracy didn't work.
% Further work has to be done in advance of the quantisation to increase the accuracy after quantisation.
...
% When the functionality of the \acrshort{dfc} of compiling the whole model is present, other measures to increase the accuracy before quantisation can be done like finetuning or the usage of the CLIP adapter\cite{clipadapter}.
